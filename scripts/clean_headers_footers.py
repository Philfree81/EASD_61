...
def process_single_abstract(
    ...
    # -----------------------------------------------------------------------
    # Institutions (fenêtre globale basée sur indice + AUTHOR_FONT)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Post-pass : institutions par ligne avec indice
    # -----------------------------------------------------------------------
    line_map = group_by_line(
        [e for e in span_elems if e.get("abstract_id") == abstract_id]
    )
    for lid, line_elems in line_map.items():
        if any(e.get("element_type") == "indice" for e in line_elems):
            for e in line_elems:
                if (
                    e.get("type") == "text"
                    and e.get("signature") == AUTHOR_FONT
